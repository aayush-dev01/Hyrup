...
export const sessionsRouter = router({
  ...
  start: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const session = await ctx.db.session.findUnique({
        where: { id: input.sessionId },
        include: { booking: { include: { teacher: true, learner: true, payment: true } } },
      });
      if (!session) throw new TRPCError({ code: "NOT_FOUND", message: "Session not found" });
      ...
      if (apiKey) {
        ...
        const teacherName = session.booking.teacher.userId;
        const learnerName = session.booking.learner.userId;
        ...
        if (text.trim()) {
          await ctx.db.sessionSummary.upsert({
            ...
            create: {
              ...
              actionItems: {
                create: [
                  { task: "Review the session summary and highlight 3 key takeaways." },
                  { task: "Write down one question to bring to the next session." },
                  { task: "Complete the first action item within 48 hours." },
                ],
              },
            },
            update: { aiGeneratedNotes: text },
          });
        }
      }
