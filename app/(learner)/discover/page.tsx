...
const TEACHERS = [
  { name: "Maya Krishnan", specialty: "UX Research", topics: ["Design", "Product"], rate: "₹1,200", nextSlot: "Tomorrow, 3:00 PM", tilt: -1.0 },
  { name: "David Chen", specialty: "System Design", topics: ["Engineering"], rate: "₹1,500", nextSlot: "Wed, 11:00 AM", tilt: 1.5 },
  { name: "Sarah Okafor", specialty: "Copywriting", topics: ["Writing", "Marketing"], rate: "₹900", nextSlot: "Thu, 2:00 PM", tilt: -0.5 },
  { name: "James Park", specialty: "Financial Modeling", topics: ["Finance"], rate: "₹1,800", nextSlot: "Fri, 10:00 AM", tilt: 1.0 },
  { name: "Priya Nair", specialty: "Product Strategy", topics: ["Product", "Leadership"], rate: "₹1,400", nextSlot: "Mon, 4:00 PM", tilt: -1.5 },
  { name: "Alex Rivera", specialty: "Motion Design", topics: ["Design", "Engineering"], rate: "₹1,100", nextSlot: "Tue, 1:00 PM", tilt: 0.5 },
];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-stretch">
        <AnimatePresence>
          {filtered.map((teacher, i) => (
            ...
            >
              <SketchCard tilt={teacher.tilt} className="p-6 flex flex-col justify-between h-full w-full">
                ...
                <div className="flex flex-col items-center">
                  <div className="relative mt-[20px] mb-4">
                    ...
                    <ProfileAvatar seed={teacher.name} size={64} className="relative z-10" />
                  </div>
                  
                  <h3 className="text-[20px] font-bold text-ink text-center mb-1">{teacher.name}</h3>
                  ...
                  <div className="flex flex-wrap justify-center gap-2 mb-4">
                    {teacher.topics.map((topic) => (
                      <span key={topic} className="relative px-3 py-1 text-[12px] text-ink-muted flex items-center justify-center">
                        {topic}
                  ...
                  <p className="text-[12px] text-ink-faint text-center mb-4">Next: {teacher.nextSlot}</p>
                </div>

                <div className="flex flex-col w-full">
                  <SketchDivider className="mb-4" />
                  <div className="flex justify-center items-baseline mb-4">
                    <span className="font-hand font-bold text-[22px] text-ink leading-none">{teacher.rate}</span>
                    <span className="text-[13px] text-ink-muted leading-none ml-1">/session</span>
                  </div>
                  <SketchButton variant="ghost" className="w-full !px-4 !py-2" href={`/book/${teacher.name.toLowerCase().replace(' ', '-')}`}>
                    Book a session
                  </SketchButton>
                </div>
              </SketchCard>
