...
export function resolvePostAuthRoute(user: UserLike, isSignIn: boolean = false) {
  if (user.role === "TEACHER") {
    return isSignIn || isTeacherOnboarded(user) ? "/teacher-dashboard" : "/onboarding/teacher-setup";
  }
  if (user.role === "LEARNER") {
    return isSignIn || isLearnerOnboarded(user) ? "/dashboard" : "/onboarding/learner-setup";
  }
  return "/dashboard";
}
